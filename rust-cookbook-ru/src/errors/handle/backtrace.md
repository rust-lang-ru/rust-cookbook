...
Этот рецепт показывает, как обрабатывать сложный сценарий ошибки, а затем напечатать обратную трассировку. Он полагается на [`chain_err`](https://docs.rs/error-chain/*/error_chain/index.html#chaining-errors) для расширения ошибок путем добавления новых ошибок. Стек ошибок можно развернуть, что обеспечивает лучший контекст для понимания причины возникновения ошибки.

Приведенные ниже рецепты пытаются десериализовать значение `256` в тип `u8`. Ошибка всплывет из Serde, затем csv и наконец к коду пользователя.
