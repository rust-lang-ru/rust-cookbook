...
Пример использует [`ring::pbkdf2`] чтобы хешировать "солёный" пароль, используя функцию вывода ключа PBKDF2 [`pbkdf2::derive`].
Проверяет корректность хеш-кода с помощью метода [`pbkdf2::verify`].
Соль генерируется используя [`SecureRandom::fill`], которая заполняет байтовый массив для соли с помощью криптографического генератора случайных чисел.
